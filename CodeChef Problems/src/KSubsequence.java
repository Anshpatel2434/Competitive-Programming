...
public class KSubsequence {
    ...
    private static void solve(PrintWriter out) {
        ...
        if (arr[0] == sorted[n - 1] || arr[n - 1] == sorted[n - 1] || n == k) {
            int counter = n - 1;
            long sum = 0;
            for (int i = 0; i < k-1; i++) {
        ...
        //check for if largest is 1st
        long sumOfTop3 = 0;
        int maxTillLargest = 0, lowestInSort = sorted[n - k];
        boolean gotLowerBefore = false;
        for (int i = 0; i < n; i++) {
            ...
            if (arr[i] >= lowestInSort) {
                gotLowerBefore = true;
                break;
            }

            maxTillLargest = Math.max(maxTillLargest, arr[i]);
        }
//        System.out.println("maxtill: "+maxTillLargest);
//        System.out.println("lowerbefore : "+gotLowerBefore);
//        System.out.println("lowestINsort: "+lowestInSort);

        if (!gotLowerBefore) {
            sumOfTop3 = maxTillLargest + sorted[n - 1] + sorted[n - 1] + sorted[n-2];
            int counter = n - 1;
            long sum = 0;
//            System.out.println("sum of top 3"+sumOfTop3);
            if (sumOfTop3 > sorted[n - 1] + sorted[n - 2] + sorted[n - 2] + sorted[n-3]) {
                for (int i = 0; i < k - 1; i++) {
                    sum += sorted[counter--];
                }
                System.out.println(sum + maxTillLargest + sorted[n-1]);
            } else {
                for (int i = 0; i < k; i++) {
                    sum += sorted[counter--];
                    if(i != 0){
                        sum += sorted[counter+1];
                    }
                }
                System.out.println(sum);
            }
        ...
        //check if the largest is last
        sumOfTop3 = 0;
        maxTillLargest = 0;
        lowestInSort = sorted[n - k];
        gotLowerBefore = false;
        for (int i = n - 1; i >= 0; i--) {
