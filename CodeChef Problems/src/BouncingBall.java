...
public class BouncingBall {
    ...
    private static void solve(PrintWriter out) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if(al.get(al.size()-1) != 0 && num != 0){
                long last = al.remove(al.size()-1);
                al.add(last + num);
                continue;
            }
            if(al.get(al.size()-1) != 0 && num == 0){
        ...
        for (int i = 0; i < al.size(); i++) {
            if(i == 0){
                prefixSum.add(al.get(i));
                continue;
            }
            if(al.get(i) != 0){
                prefixSum.add(prefixSum.get(prefixSum.size()-1) + al.get(i));
            }
        }

        List<Long> suffixSum = new ArrayList<>();
        for (int i = al.size()-1; i >= 0; i--) {
            if(i == al.size()-1){
                suffixSum.add(al.get(i));
                continue;
            }
            if(al.get(i) != 0){
                suffixSum.add(0,suffixSum.get(0) + al.get(i));
            }
        }
...
//        System.out.println(suffixSum);
        ...
        long consecutiveZeroes = 0, count = 0;
        int index = -1;
        for (int i = 0; i < al.size(); i++) {
            if(al.get(i) == 0){
                consecutiveZeroes++;
            ...
            index++;
            if(index == 0) continue;
            if(Objects.equals(prefixSum.get(index - 1), suffixSum.get(index))){
                count += consecutiveZeroes * 2;
                consecutiveZeroes = 0;
            }
            else{
                if(Math.abs(prefixSum.get(index-1) - suffixSum.get(index)) == 1){
                    count += consecutiveZeroes;
                    consecutiveZeroes = 0;
                }
            }

            if(index == prefixSum.size()-1) break;
        }
        out.println(count);
        out.flush();
    }
