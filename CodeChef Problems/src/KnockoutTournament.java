...
public class KnockoutTournament {
    ...
    private static void solve(PrintWriter out) {
        ...
        for (int i = 0; i < al.size(); i++) {
            if((i+1) == 1) map.put(al.get(i), 0);
            if((i+1) <= 3 && (i+1) >= 2) map.put(al.get(i), 1);
            if((i+1) <= 7 && (i+1) >= 4) map.put(al.get(i), 2);
            if((i+1) <= 15 && (i+1) >= 8) map.put(al.get(i), 3);
            if((i+1) == 16) map.put(al.get(i),4);
        }
        for (Integer el: al1) out.print(map.get(el)+ " ");
        out.println();
        out.flush();
    }
