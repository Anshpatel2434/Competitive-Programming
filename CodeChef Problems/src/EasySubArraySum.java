...
public class EasySubArraySum {
    ...
    private static void solve(PrintWriter out) {
        ...
        int[] arr = new int[n];
        int count = 0;
        int firstPosIndex = -1, lastPostIndex = -1;
        for (int i = 0; i < n; i++) {
            arr[i] = in.nextInt();
            if(arr[i] > 0 && firstPosIndex == -1) firstPosIndex = i;
        }

        for (int i = n-1; i >= 0; i--) {
            if(arr[i] > 0 && lastPostIndex == -1){
                lastPostIndex = i;
                break;
            }
        }
        if(lastPostIndex == -1 && firstPosIndex == -1){
            System.out.println(0);
            return;
        }

        if(lastPostIndex == firstPosIndex){
            System.out.println(0);
            return;
        }

        for (int i = firstPosIndex; i <= lastPostIndex; i++) {
            if(arr[i] < 0) count++;
        }
        System.out.println(count);
//        int countNegs = 0, maxSubArraySum = 0;
//        int[] dp = new int[n];
//        if (arr[0] < 0) {
//            countNegs = 1;
//            dp[0] = 0;
...
//        System.out.println((max <= 0)? 0 : countNegs);
    }
