...
public class TrickOrTreat {
    ...
    private static void solve(PrintWriter out) {
        ...
        Map<Integer, Integer> chocolateMap = new HashMap<>();

        long count = 0, candyDirect = 0, chocolateDirect = 0;
        for (int i = 0; i < n; i++) {
            ...
            if(candy % m == 0){
                ...
                int rem = candy % m;
                if(candyMap.containsKey(rem)){
                    int temp = candyMap.get(rem);
                    candyMap.put(rem, ++temp);
                }
                else{
                    candyMap.put(rem, 1);
                }
        ...
        for (int i = 0; i < n; i++) {
            ...
            if(chocolate % m == 0){
                ...
                int rem = chocolate % m;
                if(chocolateMap.containsKey(rem)){
                    int temp = chocolateMap.get(rem);
                    chocolateMap.put(rem, ++temp);
                }
                else{
                    chocolateMap.put(rem, 1);
                }
            }
        }

        if(candyDirect == n && chocolateDirect == n){
            out.println(candyDirect*chocolateDirect);
            out.flush();
            return;
        }
        ...
        for (Integer key:
             candyMap.keySet()) {
            int req = m - key;
            if(chocolateMap.containsKey(req)){
                count += ((long) candyMap.get(key) * chocolateMap.get(req));
            }
        }

        count += (candyDirect * chocolateDirect);
        out.println(count);
