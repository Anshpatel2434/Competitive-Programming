...
public class VladAndDivision {
    ...
    private static void solve(PrintWriter out) {
        ...
        for (int i = 0; i < n; i++) {
            int num = arr[i];
            if (freq.get(num) == null || freq.get(num) == 0) continue;

            int onesComplement = num^(int)(Math.pow(2,31));
//            System.out.println("Num is : "+num);
//            System.out.println("Not num is : "+onesComplement);
            if (freq.containsKey(onesComplement) && freq.get(onesComplement) > 0) {
                freq.put(onesComplement, freq.get(onesComplement) - 1);
                freq.put(num, freq.get(num) - 1);
                count++;
            } else if(freq.get(num) > 0) {
                freq.put(num, freq.get(num) - 1);
                count++;
            }
        }
