    ...
    private static void solve(PrintWriter out) {
        ...
        for (int i = 0; i < n; i++) {
            if (Character.isLowerCase(s.charAt(i))) {
                lowerFreq[s.charAt(i) - 97]++;
            } else if (Character.isUpperCase(s.charAt(i))) {
                upperFreq[s.charAt(i) - 65]++;
            }
        }
        ...
        for (int i = 0; i < 26; i++) {
            if (lowerFreq[i] > upperFreq[i]) {
                if(lowerFreq[i] != 0 && upperFreq[i] != 0){
                    int score = upperFreq[i];
                    ans += score;
                    lowerFreq[i] -= score;
                    upperFreq[i] -= score;
                }
                if(k > 0){
                    if (lowerFreq[i] - upperFreq[i] > 1) {
                        int change = (lowerFreq[i] - upperFreq[i]) / 2;
                        if (change < k) {
                            ans += change;
                            k -= change;
                        } else {
            ...
            } else if (upperFreq[i] > lowerFreq[i]) {
                if(lowerFreq[i] != 0 && upperFreq[i] != 0){
                    int score =lowerFreq[i];
                    ans += score;
                    lowerFreq[i] -= score;
                    upperFreq[i] -= score;
                }
                if(k > 0){
                    if (upperFreq[i] - lowerFreq[i] > 1) {
                        int change = (upperFreq[i] - lowerFreq[i]) / 2;
                        if (change < k) {
                            ans += change;
                            k -= change;
                        } else {
            ...
            } else if(upperFreq[i] != 0 && lowerFreq[i] != 0){
                int score = upperFreq[i];
                ans += score;
            }
        }
