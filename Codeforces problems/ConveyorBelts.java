...
public class ConveyorBelts {
    ...
    private static void solve(PrintWriter out) {
        ...
        long x1Start = Math.min(x1, n-x1);
        long y1Start = Math.min(y1, n-y1);
        long x2Start = Math.min(x2, n-x2);
        long y2Start = Math.min(y2, n-y2);

        long belt1Start = Math.min(x1Start,y1Start);
        long belt2Start = Math.min(x2Start, y2Start);
        out.println(Math.abs(belt1Start-belt2Start));
