...
public class TilesComeback {
    ...
    private static void solve(PrintWriter out) {
        ...
        int start = 0,end = n-1,firstPosNum = c[0],lastPosNum = c[n-1];
        int firstPosCount = 0, lastPosCount = 0;
        boolean stopEnd = false, stopStart = false;
        if(firstPosNum == lastPosNum){
            for (int i = 0; i < n; i++) {
                if(firstPosCount != k){
                    if(c[i] == firstPosNum){
                        firstPosCount++;
            ...
            while(start <= end){
                if(stopEnd && stopStart) break;
                if(firstPosCount != k){
                    if(c[start] == firstPosNum){
                        firstPosCount++;
                    }
                }
                else{
            ...
            if(firstPosCount == k && lastPosCount == k){
                out.println("YES");
            }
            else{
                out.println("NO");
