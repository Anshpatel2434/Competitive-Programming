...
public class MaxMexCut {
    ...
    private static void solve(PrintWriter out) {
        ...
        for (int i = 0; i < n; i++) {
            arr[0][i] = (s1.charAt(i) == '0') ? 0 : 1;
        }

        for (int i = 0; i < n; i++) {
            arr[1][i] = (s2.charAt(i) == '0') ? 0 : 1;
        }
        ...
        for (int i = 0; i < n; i++) {
            int xor = arr[0][i] ^ arr[1][i];
            int and = arr[0][i] & arr[1][i];
            if (xor == 1) {
                sum += 2;
                if (consecutiveOne > 0) {
                    sum += consecutiveOne;
            ...
            if(and == 1 && consecutiveOne == 0){
                findZero = true;
            }

            if (and == 1) {
                if(consecutiveOne > 0){
                    sum += (consecutiveOne - 1);
                    sum += 2;
                    consecutiveOne = 0;
                }
            }
        ...
        out.println(sum + consecutiveOne);
        out.flush();
    }
