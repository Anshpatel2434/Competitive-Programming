...
public class ArrayMerging {
    ...
    private static void solve(PrintWriter out) {
        ...
        int prev = 0, currentCount = 0;
        for (int i = 0; i < n; i++) {
            a[i] = in.nextInt();
            if (i == 0) {
                currentCount++;
                prev = a[i];
                mapa.put(a[i], currentCount);
                continue;
            }
            if (a[i] == prev) {
                currentCount++;
                if (i == n - 1) {
                    if (mapa.containsKey(prev)) {
                        if (mapa.get(prev) < currentCount) {
                            mapa.put(prev, currentCount);
                        }
                    } else {
                        mapa.put(prev, currentCount);
                    }
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (i == 0) {
                currentCount++;
                prev = b[i];
                mapb.put(b[i], currentCount);
                continue;
            }
            if (b[i] == prev) {
                currentCount++;
                if (i == n - 1) {
                    if (mapb.containsKey(prev)) {
                        if (mapb.get(prev) < currentCount) {
                            mapb.put(prev, currentCount);
                        }
                    } else {
                        mapb.put(prev, currentCount);
                    }
...
//        System.out.println(mapb.toString());
        ...
        for (Integer freq :
                mapa.keySet()) {
            if (mapa.containsKey(freq) && mapb.containsKey(freq)) {
                int tempSum = mapa.get(freq) + mapb.get(freq);
                max = Math.max(max, tempSum);
            } else max = Math.max(max, mapa.get(freq));
        }
        for (Integer freq :
                mapb.keySet()) {
            if (mapa.containsKey(freq) && mapb.containsKey(freq)) {
                int tempSum = mapa.get(freq) + mapb.get(freq);
                max = Math.max(max, tempSum);
            } else max = Math.max(max, mapb.get(freq));
        }
