...
public class LyoTianyiAndTheTable {
    ...
    private static void solve(PrintWriter out) {
        ...
        Arrays.sort(arr);
        int highSum1 = arr[len-1] - arr[0];
        int highSum2 = arr[len-1] - arr[1];
        int smallSum1 = arr[len-1] - arr[0];
        int smallSum2 = arr[len-2] - arr[0];

        if(highSum1+highSum2 > smallSum1+smallSum2){
            if (m > n) {
                System.out.println(((m - 1) * (n) * highSum1) + (highSum2 * (n - 1)));
            }
            if (n >= m) {
                System.out.println(((n - 1) * (m) * highSum1) + (highSum2 * (m - 1)));
            }
        } else{
            if (m > n) {
                System.out.println(((m - 1) * (n) * smallSum1) + (smallSum2 * (n - 1)));
            }
            if (n >= m) {
                System.out.println(((n - 1) * (m) * smallSum1) + (smallSum2 * (m - 1)));
            }
        }
    }
