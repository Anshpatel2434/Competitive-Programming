...
public class ClosestCities {
    ...
    private static void solve(PrintWriter out) {
        ...
        for (int i = 1; i < n-1; i++) {
            if(arr[i + 1] - arr[i] < arr[i] - arr[i - 1]) prefixSum[i+1] = prefixSum[i] + 1;
            else prefixSum[i+1] = prefixSum[i] + (arr[i + 1] - arr[i]);
        }

        suffixSum[n-1] = 0;
        suffixSum[n-2] = 1;
        for (int i = n-2; i >= 1; i--) {
            if(arr[i] - arr[i - 1] < arr[i + 1] - arr[i]) suffixSum[i - 1] = suffixSum[i] + 1;
            else suffixSum[i - 1] = suffixSum[i] + (arr[i] - arr[i - 1]);
        }
        ...
        for (int i = 0; i < m; i++) {
            int x = in.nextInt();
            int y = in.nextInt();
            if(x < y) out.println(prefixSum[y-1] - prefixSum[x-1]);
            else out.println(suffixSum[y-1] - suffixSum[x-1]);
        }
        out.println();
        out.flush();
    }
