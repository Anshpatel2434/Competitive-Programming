...
public class FindTheDifferentOnes {
    ...
    private static void solve(PrintWriter out) {
        ...
        int[] lastPairForDiff = new int[n + 1];
        int[] arr = new int[n + 1];
        int prev = 0;
        int leftIndex = -1, leftDiffIndex = -1;
        for (int i = 1; i <= n; i++) {
            arr[i] = in.nextInt();
            if (i == 1) {
                prev = arr[i];
            ...
            lastPairForDiff[i] = leftDiffIndex;
            pairs[i] = leftIndex;
            prev = arr[i];
        }
//        out.println(Arrays.toString(pairs));
//        out.println(Arrays.toString(lastPairForDiff));
        int q = in.nextInt();
        for (int i = 0; i < q; i++) {
            int l = in.nextInt();
            int r = in.nextInt();
            if (pairs[r] == pairs[l]) {
                ...
                if (arr[l] == arr[r]) {
                    out.println(lastPairForDiff[r] + " " + r);
                } else {
                    out.println(l + " " + r);
                }
            }
