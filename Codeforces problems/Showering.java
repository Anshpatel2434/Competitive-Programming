...
public class Showering {
    ...
    private static void solve(PrintWriter out) {
        ...
        int s = in.nextInt();
        int m = in.nextInt();
        int maxInterval = Integer.MIN_VALUE, prevRight = 0;
        for (int i = 0; i < n; i++) {
            int l = in.nextInt();
            int r = in.nextInt();
            maxInterval = Math.max(maxInterval, (l - prevRight));
            prevRight = r;
        }

        maxInterval = Math.max(maxInterval, (m - prevRight));

        out.println((maxInterval >= s)? "YES" : "NO");
        out.flush();
    }
