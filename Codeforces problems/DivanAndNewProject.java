...
public class DivanAndNewProject {
    ...
    private static void solve(PrintWriter out) {
        ...
        int h = (n+1)/2, forward = h+1, backward = h-1;
        long ans = 0;
        int ptr = 0;
        for (int i = 0; i < keys.size(); i++) {
            int curr = keys.get(i);
            ArrayList<Integer> currIndices = map.get(curr);
            for (int j = 0; j < currIndices.size(); j++) {
                if(ptr % 2 == 0){
                    ans += 2L * (long)Math.abs((h - forward)) * (long)curr;
                    int index = currIndices.get(j);
                    arr[index] = forward++;
                }
                else{
                    ans += 2L * (long)Math.abs((h - backward)) * (long)curr;
                    int index = currIndices.get(j);
                    arr[index] = backward--;
                }
                ptr++;
