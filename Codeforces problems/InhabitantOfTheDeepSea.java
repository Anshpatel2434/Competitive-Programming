...
public class InhabitantOfTheDeepSea {
    ...
    private static void solve(PrintWriter out) {
        ...
        for (int i = 0; i < n; ++i) {
            dq.addLast(in.nextLong());
        }

        while (dq.size() > 1 && k > 0) {
            long mn = Math.min(dq.peekFirst(), dq.peekLast());
            if (k < 2 * mn) {
                dq.addFirst(dq.removeFirst() - (k / 2 + k % 2));
                dq.addLast(dq.removeLast() - (k / 2));
                k = 0;
            } else {
                dq.addFirst(dq.removeFirst() - mn);
                dq.addLast(dq.removeLast() - mn);
                k -= 2 * mn;
            }
            if (dq.getFirst() == 0) {
                dq.removeFirst();
