...
public class OlyaAndTheGameOfArrays {
    ...
    private static void solve(PrintWriter out) {
        ...
        int n = in.nextInt();
        int minN = Integer.MAX_VALUE, min2 = Integer.MAX_VALUE;
        long sum = 0;
        for (int i = 0; i < n; i++) {
            int m = in.nextInt();
            ...
            Collections.sort(al);
            int tempMin =al.get(0);
            minN = Math.min(tempMin, minN);
            al.removeFirst();
            int tempMin2 = al.get(0);
            sum += tempMin2;
            min2 = Math.min(min2, tempMin2);
        }
        out.println(minN + sum - min2);
        out.flush();
    }
