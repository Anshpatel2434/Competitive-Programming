...
public class GoingToTheCinema {
    ...
    private static void solve(PrintWriter out) {
        ...
        int partners = 0, prev = 0, ans = 0;

        if (!keys.contains(Integer.valueOf(0))) {
            ans++;
        }

        for (int i = 0; i < keys.size() - 1; i++) {
            partners = (freq.get(keys.get(i)) - 1) + prev;
            if (partners >= keys.get(i) && partners + 1 < keys.get(i + 1)) {
                ans++;
            }
            prev += freq.get(keys.get(i));
        }
        //for last
        partners = (freq.get(keys.get(keys.size() - 1)) - 1) + prev;
        if (partners >= keys.get(keys.size() - 1)) {
            ans++;
        }
        out.println(ans);
        out.flush();
