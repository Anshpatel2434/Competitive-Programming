...
public class JellyfishAndUndertale {
    ...
    private static void solve(PrintWriter out) {
        ...
        sum += b - 1;
        int timer = 1;
//        out.println("Initially the seconds went to reach 1 sec is: "+sum);
        for (int i = 0; i < n; i++) {
            int temp = in.nextInt();
            if(temp < a){
                timer += temp;
...
//            out.println("Now the timer is at : "+timer);
            sum += timer - 1;
            timer--;
//            out.println("Now removing that current second the timer is at : "+timer);
//            out.println("REmoving the extra seconds to reach 1 second and the sum is : "+sum);
            timer = 1;
        }
        out.println(sum+1);
        out.flush();
