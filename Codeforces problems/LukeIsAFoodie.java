...
public class LukeIsAFoodie {
    ...
    private static void solve(PrintWriter out) {
        ...
        int[] arr = new int[n];
        ArrayInput(arr);
        int prevMin = arr[0], prevMax = prevMin;

        for (int i = 1; i < n; i++) {
            int curr = arr[i];
            if(curr > prevMin){
                if(Math.abs(curr - prevMin) > 2 * x){
                    prevMin = curr;
                    prevMax = curr;
                    count++;
                    continue;
            ...
            prevMin = Math.min(curr, prevMin);
            prevMax = Math.max(curr, prevMax);
//            System.out.println("---------");
//            System.out.println("for curr : "+curr);
//            System.out.println("prev min changed to : "+prevMin);
//            System.out.println("and count increased to : "+count);
        }
