...
public class GameWithMultiset {
    ...
    private static void solve(PrintWriter out) {
        ...
        Map<Long, Long> twoPowerFreq = new HashMap<>();
        
        for (int i = 0; i < m; i++) {
            int t = in.nextInt();
            long v = in.nextLong();
            if(t == 1){
                twoPowerFreq.put((long)Math.pow(2, v), twoPowerFreq.getOrDefault((long)Math.pow(2, v), 0L)+1);
            }
            else{
                for (int j = 30; j >= 0; j--) {
                    long curr = (long)Math.pow(2, j);
                    if(twoPowerFreq.containsKey(curr) && v != 0){
                        if(curr <= v){
                            v -= (Math.min((v / curr), twoPowerFreq.get(curr)) * curr);
                        }
                    }
                }
                if(v == 0) System.out.println("YES");
    ...
    static boolean decToBinary(long n, int[] bits) {
        ...
        while (n > 0) {
            ...
            System.out.println("bits[i] is : "+bits[i]);
            if (binaryNum[i] == 1 &&  bits[i] == 0) {
                return false;
            }
            i++;
        }
