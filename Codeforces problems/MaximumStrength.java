...
//
//        String s = Double.toString(da);
//        String t = Double.toString(db);
...
//        t = t.substring(0, t.length() - 2);
        ...
        while (!s.isEmpty() && s.charAt(0) == t.charAt(0)) {
            s = s.substring(1);
            t = t.substring(1);
        }

        int ans = 9 * Math.max(0, s.length() - 1);
        if (!s.isEmpty() && !t.isEmpty()) {
            ans += Math.abs(Integer.parseInt(""+t.charAt(0)) - Integer.parseInt(""+s.charAt(0)));
        }
