...
public class GCDCompression {
    ...
    private static void solve(PrintWriter out) {
        ...
        for (int i = 0; i < arr.length; i++) {
            if(in.nextInt() % 2 == 0)evenIndices.add(i + 1);
            else oddIndices.add(i + 1);
        }

        for (int i = 0; i < n - 1; i++) {
            if(evenIndices.size() >= 2){
                out.println(evenIndices.removeFirst() + " "+ evenIndices.removeFirst());
                continue;
            }
            if(oddIndices.size() >= 2){
                out.println(oddIndices.removeFirst() + " " + oddIndices.removeFirst());
            }
        }
        out.flush();
    }
