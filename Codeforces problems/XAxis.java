...
public class XAxis {
    ...
    private static void solve(PrintWriter out) {
        ...
        int sumOf12 = Math.abs(x1 - x3) + Math.abs(x2 - x3);
        int sumOf13 = Math.abs(x1 - x2) + Math.abs(x3 - x2);
        int sumOf32 = Math.abs(x2 - x1) + Math.abs(x3 - x1);

        out.println(Math.min(sumOf12, Math.min(sumOf32, sumOf13)));
        out.flush();
    }
