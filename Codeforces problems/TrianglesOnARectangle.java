...
public class TrianglesOnARectangle {
    ...
    private static void solve(PrintWriter out) {
        ...
        long h = in.nextLong();
        int kx1= in.nextInt();
        long[] x1 = new long[kx1];
        ArrayInput(x1);

        int kx2= in.nextInt();
        long[] x2 = new long[kx2];
        ArrayInput(x2);

        int ky1= in.nextInt();
        long[] y1 = new long[ky1];
        ArrayInput(y1);

        int ky2= in.nextInt();
        long[] y2 = new long[ky2];
        ArrayInput(y2);

        long xMax = Math.max((x1[kx1-1] - x1[0]) * h,(x2[kx2-1] - x2[0]) * h);
        long yMax = Math.max((y1[ky1-1] - y1[0]) * w,(y2[ky2-1] - y2[0]) * w);

        out.println(Math.max(xMax, yMax));
